 ...
 */
export function useTriggerSetup(initialEventId: GitHubTriggerEventId = "github.issue.created") {
  ...
  const selectRepository = useCallback(
    (repository: RepositorySelection) => {
      ...
      if (isTriggerRequiringCallsign(step.state === "select-repository" ? step.eventId : eventId)) {
        setStep({
          state: "input-callsign",
  ...
  const completeCallsignInput = useCallback(
    (callsign: string) => {
      if (step.state !== "input-callsign") {
        throw new Error("Cannot complete callsign input: not in input-callsign state");
      }
